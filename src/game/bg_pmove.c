...
//	}
  ...
  if( pm->ps->weapon == WP_BLASTER )
  {
    ...
    else
    {
      ...
      pm->ps->delta_angles[ YAW ] -= ANGLE2SHORT( ( ( random() * 25) -12.5 ) * ( 30.0 / (float)addTime ) );
    }
  }
  //Rifle
  if( pm->ps->weapon == WP_MACHINEGUN )
  {
    ...
    {
      pm->ps->delta_angles[ PITCH ] -= ANGLE2SHORT( ( ( random() * 12 ) - 4 ) * ( 30.0 / (float)addTime ) );
      pm->ps->delta_angles[ YAW ] -= ANGLE2SHORT( ( ( random() * 8 ) - 4 ) * ( 30.0 / (float)addTime ) );
    }
  //FIXME: doesn't work
...
void Pmove( pmove_t *pmove )
{
  ...
  while( pmove->ps->commandTime != finalTime )
  {
    ...
    {
      if( msec > 66 ) //basically minimum fps of 15
        msec = 66;
    }
