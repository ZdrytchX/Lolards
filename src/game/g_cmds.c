    ...
    case SAY_TEAM:
     ...
     // while ( sscanf( tmptext, "%s %s", strprint, tmptext) > 0 ) {
        ...
        /*if( !Q_stricmpn(tmptext, "follow me", 9) && ent->client->pers.teamSelection != PTE_NONE )
        {
            ...
            //G_SanitiseString( botname, botname_s, sizeof( botname_s ) );
            botname_s = Q_CleanStr( botname );
            G_ClientNumbersFromString( botname_s, pids);
...
//   {
  ...
  if(G_SayArgc( ) != 2 && G_SayArgc( ) != 3)
  {
    ...
    for(i = 0;i < numnearby; i++)
    {
      trap_SendServerCommand( ent-g_entities, va("print \"^5Nearby Node: %d ^2(X:%f Y:%f Z:%f) ^1%d %d %d %d %d ^3Random:^7%d ^3Timeout:^7%d ^3Action:^7%d\n\"", nearbynodeid[i],
        level.nodes[nearbynodeid[i]].coord[0],
        level.nodes[nearbynodeid[i]].coord[1],
        level.nodes[nearbynodeid[i]].coord[2],
        level.nodes[nearbynodeid[i]].nextid[0],
        level.nodes[nearbynodeid[i]].nextid[1],
        level.nodes[nearbynodeid[i]].nextid[2],
        level.nodes[nearbynodeid[i]].nextid[3],
        level.nodes[nearbynodeid[i]].nextid[4],
        level.nodes[nearbynodeid[i]].random,
        level.nodes[nearbynodeid[i]].timeout,
        level.nodes[nearbynodeid[i]].action) );
    }
    return;
  }
  if( G_SayArgc() == 2 || G_SayArgc() == 3 )
  {
    ...
    if( G_SayArgc( ) == 3 && !Q_stricmp( cmd, "timeout" ) )
    {
      ...
      level.nodes[nearbynodeid[0]].timeout = timeout2;
    ...
    else if(G_SayArgc( ) == 3 && !Q_stricmp( cmd, "action" ))
    {
      ...
      {
        level.nodes[nearbynodeid[0]].action = BOT_JUMP;
      }
      else if(!Q_stricmp( action, "wallclimb" ))
      {
        level.nodes[nearbynodeid[0]].action = BOT_WALLCLIMB;
      }
      else if(!Q_stricmp( action, "kneel" ))
      {
        level.nodes[nearbynodeid[0]].action = BOT_KNEEL;
      }
      else if(!Q_stricmp( action, "pounce" ))
      {
        level.nodes[nearbynodeid[0]].action = BOT_POUNCE;
      }
      else if(!Q_stricmp( action, "none" ))
      {
        level.nodes[nearbynodeid[0]].action = 0;
      }
      else
      {
      ...
      else if( !Q_stricmp( cmd, "add" ) )
      {
        ...
        if( pathfound == qtrue )
        {
          level.nodes[i].coord[0] = ent->s.pos.trBase[0];
          level.nodes[i].coord[1] = ent->s.pos.trBase[1];
          level.nodes[i].coord[2] = ent->s.pos.trBase[2];
          level.nodes[i].nextid[0] = 1000 + MAX_NODES;
          level.nodes[i].nextid[1] = 1000 + MAX_NODES;
          level.nodes[i].nextid[2] = 1000 + MAX_NODES;
          level.nodes[i].nextid[3] = 1000 + MAX_NODES;
          level.nodes[i].nextid[4] = 1000 + MAX_NODES;
          level.nodes[i].random = 0;
      ...
      else if( !Q_stricmp( cmd, "connect" ) )
      {
        ...
        else if( ent->pathid > -1 && ent->pathid < level.numNodes )
        {
          ...
          for( i = 0; i < 5; i++ )
          {
            ...
            if( level.nodes[nearbynodeid[0]].nextid[i] == ent->pathid )
            {
              linked = qtrue;
            }
            if( level.nodes[ent->pathid].nextid[i] == nearbynodeid[0] )
            {
              linked2 = qtrue;
            }
          }
          ...
          for( i = 0; i < 5; i++ )
          {
            if( level.nodes[nearbynodeid[0]].nextid[i] >= MAX_NODES )
            {
              countz = i;
              i = 5;
              break;
          ...
          for( i = 0; i < 5; i++ )
          {
            if( level.nodes[ent->pathid].nextid[i] >= MAX_NODES )
            {
              countz2 = i;
              i = 5;
              break;
          ...
          if( linked == qtrue )
          {
            ...
            level.nodes[ent->pathid].nextid[countz2] = nearbynodeid[0];
          ...
          if( linked2 == qtrue )
          {
            ...
            level.nodes[nearbynodeid[0]].nextid[countz] = ent->pathid;
      ...
      else if( !Q_stricmp( cmd, "clear" ) )
      {
        ...
        level.nodes[nearbynodeid[0]].nextid[0] = 1000 + MAX_NODES;
        level.nodes[nearbynodeid[0]].nextid[1] = 1000 + MAX_NODES;
        level.nodes[nearbynodeid[0]].nextid[2] = 1000 + MAX_NODES;
        level.nodes[nearbynodeid[0]].nextid[3] = 1000 + MAX_NODES;
        level.nodes[nearbynodeid[0]].nextid[4] = 1000 + MAX_NODES;
        level.nodes[nearbynodeid[0]].random = 0;
        level.nodes[nearbynodeid[0]].timeout = 0;
        level.nodes[nearbynodeid[0]].action = 0;
      ...
      else if( !Q_stricmp( cmd, "move" ) )
      {
        ...
        else if(ent->movepathid >= 0 && ent->movepathid < level.numNodes)
        {
          ...
          level.nodes[ent->movepathid].coord[0] = ent->s.pos.trBase[0];
          level.nodes[ent->movepathid].coord[1] = ent->s.pos.trBase[1];
          level.nodes[ent->movepathid].coord[2] = ent->s.pos.trBase[2];
      ...
      else if( !Q_stricmp( cmd, "delete" ) )
      {
        ...
        for( i = 0; i < level.numNodes; i++ )
        {
          for( i2 = 0; i2 < 5; i2++ )
          {
            if( level.nodes[i].nextid[i2] == nearbynodeid[0] )
            {
              level.nodes[i].nextid[i2] = 1000 + MAX_NODES;
            }
          }
        }
        level.nodes[nearbynodeid[0]].nextid[0] = -1;
        level.nodes[nearbynodeid[0]].nextid[1] = -1;
        level.nodes[nearbynodeid[0]].nextid[2] = -1;
        level.nodes[nearbynodeid[0]].nextid[3] = -1;
        level.nodes[nearbynodeid[0]].nextid[4] = -1;
      ...
      else if( !Q_stricmp( cmd, "disconnect" ) )
      {
        ...
        else if( ent->discpathid >= 0 && ent->discpathid < level.numNodes )
        {
          ...
          for( i = 0; i < 5; i++ )
          {
            if( level.nodes[nearbynodeid[0]].nextid[i] == ent->discpathid )
            {
              level.nodes[nearbynodeid[0]].nextid[i] = 1000 + MAX_NODES;
            }
            if( level.nodes[ent->discpathid].nextid[i] == nearbynodeid[0] )
            {
              level.nodes[ent->discpathid].nextid[i] = 1000 + MAX_NODES;
            }
          }
      ...
      else if( !Q_stricmp( cmd, "random" ) )
      {
        ...
        if( level.nodes[nearbynodeid[0]].random <= 0 )
        {
          level.nodes[nearbynodeid[0]].random = 1;
          trap_SendServerCommand( ent-g_entities, va( "print \"Node #%d is set random.\n\"", nearbynodeid[0] ) );
          return;
        }
        else if( level.nodes[nearbynodeid[0]].random > 0 )
        {
          level.nodes[nearbynodeid[0]].random = 0;
